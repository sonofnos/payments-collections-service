...
- **Live**: https://sonofnos-payments-collections.onrender.com (Render free tier - sleeps after inactivity, first request can take up to a minute or so to wake it back up). `/actuator/health`, `/swagger-ui.html`, and the full API are up and DB-backed for real.
- Postgres: Render allows exactly one free-tier Postgres per account, and that slot was already held by an unrelated project (`agent-platform`) running in the same account. Rather than pay for a second instance, this service shares that same free Postgres *server* via its own separate database (`payments_collections`, owned by a role scoped to only that database) - one physical free-tier instance, multiple logical databases, which is exactly what a Postgres server is for. **Once that free-tier database's 30-day clock expires it needs manual renewal**, same as any Render free Postgres - noted here plainly.
- Redis: free Key Value instance `payments-redis`, wired via its internal (unauthenticated, private-network-only) connection.
- `CORE_BANKING_BASE_URL` points at the live core-banking-service (https://sonofnos-core-banking.onrender.com).

**Real bugs this deployment caught that local `mvn test` never did** (both fixed, both covered by new regression tests):
- jjwt's `signWith(key)` infers HS256/HS384/HS512 purely from the secret's byte length. A 64-byte secret (`openssl rand -hex 32`, a completely natural way to generate one) silently signed tokens as HS512, which the HS256-only resource-server decoder then rejected - a 401 on every request, only visible once a real secret of that length was used. Fixed by pinning the algorithm explicitly.
- A debit failure against core-banking-service triggered a second `saveAndFlush()` on a JPA entity whose in-memory reference had gone stale after the *first* `saveAndFlush()` (which goes through Hibernate's `merge()`, returning a different managed instance) - Hibernate rejected the second write as a lost update, a 500 on a completely ordinary, single-request failure path. The MockMvc test suite's mocked repository couldn't catch this because a stub `saveAndFlush` just echoes its argument back instead of exercising real `merge()` semantics. Caught by hitting the real deployed service with a debit that actually fails, reproduced locally via `docker-compose`, fixed, and covered by an integration test against real Postgres.
